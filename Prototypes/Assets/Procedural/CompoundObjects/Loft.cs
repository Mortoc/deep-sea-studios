...
namespace DSS.Procedural
{
    public class Loft
    {
        ...
        public Mesh GenerateMesh(int pathSegments, int shapeSegments)
        {
            ...
            {
                return (pathSegment * shapeSegments) + shapeSegment;
            };
            ...
            for(var pathSeg = 0; pathSeg < pathSegments+1; ++pathSeg)
            {
                ...
                for(var shapeSeg = 0; shapeSeg < shapeSegments+1; ++shapeSeg)
                {
                    ...
                    boneWeights[vertIdx] = new BoneWeight();
                    boneWeights[vertIdx].boneIndex0 = pathSeg;
                    boneWeights[vertIdx].weight0 = 1.0f;
                }
            }

            var highestVertIdx = -1;
            var triIdx = 0;
            for(var pathSeg = 0; pathSeg < pathSegments; ++pathSeg)
            {
            ...
            if( StartCap || EndCap )
            {
                ...
                if( StartCap )
                {
                    ...
                    startCapInstance.mesh.boneWeights = new BoneWeight[capVertCount];
                    for (var i = 0; i < capVertCount; )
                    {
                        startCapInstance.mesh.boneWeights[i].boneIndex0 = 0;
                        startCapInstance.mesh.boneWeights[i].weight0 = 1.0f;
                    }
                ...
                if( EndCap )
                {
                    ...
                    endCapInstance.mesh.boneWeights = new BoneWeight[capVertCount];
                    for (var i = 0; i < capVertCount; )
                    {
                        endCapInstance.mesh.boneWeights[i].boneIndex0 = pathSegments;
                        endCapInstance.mesh.boneWeights[i].weight0 = 1.0f;
                    }
        ...
        public void GenerateSkinnedMesh(int pathSegments, int shapeSegments, SkinnedMeshRenderer skin)
        {
            ...
            for (var i = 0; i < bones.Length; ++i)
            {
                ...
                if( i < existingBones.Length )
                {
                    boneObj = existingBones[i].gameObject;
                    newBone = existingBones[i];
                }
                else
                {
